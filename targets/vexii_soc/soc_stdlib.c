...
#ifdef SPRAM
static uint8_t* scratch_heap_ptr = NULL;
// TODO: We should let it act like a real malloc, with metadata
void* scratch_malloc(size_t size, uint32_t alignment){
    ...
    if (scratch_heap_ptr == NULL) {
        scratch_heap_ptr = __onchip_data_start;
    }

    // Align the current pointer
    uintptr_t addr = (uintptr_t)scratch_heap_ptr;
    uintptr_t aligned = (addr + alignment - 1) & ~(alignment - 1);

    uint8_t* ptr = (uint8_t*)aligned;
    uint8_t* new_heap_ptr = ptr + size;

    // Check if we have enough space in on-chip RAM
    if (new_heap_ptr > __onchip_data_end) {
        return NULL;
    }

    scratch_heap_ptr = new_heap_ptr;
    return ptr;
}
...
void scratch_reset(void){
    // Reset the scratch heap to start of on-chip RAM
    scratch_heap_ptr = __onchip_data_start;
}
#endif